...
CREATE OR REPLACE VIEW analysis.avg_quantity_per_order_stats AS
SELECT 
    ROUND(AVG(b.quantity), 1) AS average_quantity_per_order
FROM PUBLIC.bridge_orders_products b;

 
SELECT * FROM analysis.avg_quantity_per_order_stats;


CREATE OR REPLACE VIEW analysis.avg_order_price_stats AS
SELECT 
    ROUND(AVG(f.total_price), 1) AS average_order_price
FROM PUBLIC.fact_orders f;


SELECT * FROM analysis.avg_order_price_stats;
 

CREATE OR REPLACE VIEW analysis.unique_orders_stats AS
SELECT 
    COUNT(DISTINCT b.orderid) AS total_unique_orders
FROM PUBLIC.bridge_orders_products b;


SELECT * FROM analysis.unique_orders_stats;
